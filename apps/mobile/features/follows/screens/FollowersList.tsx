...
import { fetchMyFollowers } from '@beproud/api';

type Props = { userId: string };

export default function FollowersList({ userId }: Props) {
  const { user } = useSession();
  const queryKey = ['followers', userId] as const;
  const q = useQuery({
    queryKey,
    queryFn: () => fetchMyFollowers(userId),
  });
    ...
    <FlatList
      ...
      ListEmptyComponent={
        <View className="items-center px-6 py-16">
          <Text className="text-sm text-brand-300">Aún no tiene seguidores.</Text>
        </View>
      }
    />
  );
